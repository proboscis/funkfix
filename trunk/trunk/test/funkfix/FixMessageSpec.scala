...
class FixMessageSpec extends Spec with ShouldMatchers {
  ...
  describe("Fix message") {
    ...
    val logon = new FixMessage(List(35 -> "A", 11 -> "a", 12 -> "b", 108 -> "30"))
    val nos = new FixMessage(List(35 -> "D", 38 -> "100", 54 -> "1", 55 -> "EUR/USD", 44 -> "1.251", 40 -> "1"))
    ...
    it("should be able to extract one value with conversion") {
      val value = logon extract (Tags.HeartBtInt, toInt)
      value should be (Some(30))
    }

    it("should be able to extract two values with conversion") {
      val value = logon extract (Tags.HeartBtInt -> toInt, Tags.EncryptMethod -> keep)
      value should be ((Some(30), None))
    }

    it("should be able to extract three values at once") {
      val (qty, symbol, price) = nos extract(Tags.OrderQty -> toInt, Tags.Symbol -> keep, Tags.Price -> toDouble)
    ...
    it("should be able to extract four values at once") {
      val (qty, symbol, price, side) = nos extract(Tags.OrderQty -> toInt, Tags.Symbol -> keep, Tags.Price -> toDouble,
              Tags.Side -> ((x: String) => if(x == "1") "BUY" else "SELL"))
      qty should be (Some(100))
      symbol should be (Some("EUR/USD"))
      price should be (Some(1.251))
    ...
    it("should be able to extract five values at once") {
      val (qty, symbol, price, side, orderType) = nos extract(
              ...
              Tags.Price -> toDouble,
              Tags.Side -> ((x: String) => if(x == "1") "BUY" else "SELL"),
              Tags.OrdType -> ((x: String) => if(x == "1") "MARKET" else "LIMIT")
      )
      qty should be (Some(100))
      symbol should be (Some("EUR/USD"))
      price should be (Some(1.251))
    ...
    it("should be able to extract values with no conversion") {
      val value = logon extract (Tags.HeartBtInt, keep)
      value should be (Some("30"))
    }
  }
}